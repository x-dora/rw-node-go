...
[`panel-api.json`](panel-api.json) 保存 Go contract 测试需要的少量请求和响应样例。[`upstream-contract.sha256.json`](upstream-contract.sha256.json) 保存官方 [`remnawave/node`](https://github.com/remnawave/node) 3.0.0 的 Panel-facing contract 文件路径和 SHA-256，用于后续检查上游 contract 是否漂移。
