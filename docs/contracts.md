...
| Handler | `/node/handler/drop-users-connections`, `/node/handler/drop-ips` | adapter stub | 返回成功；不操作 conntrack 或 nftables。 |
| Stats | `/node/stats/*` | partial | system stats 已按官方 2.7.0 响应形状返回宿主机 CPU、memory、uptime、load、network interface 和 Xray sys stats；users、inbound、outbound、combined 已接入内嵌 stats feature；online status/IP 当前按系统能力降级。 |
