...
- [x] 注册官方 [`dev/2.8.0`](https://github.com/remnawave/node/tree/a5acdeb28840e21c2622a6362dc6824b6e70eea5) 计划内 Panel-facing routes。
- [x] 建立 `internal/contracts` 请求和响应类型。
- [x] 统一 response envelope。
- [~] 官方 dev/2.8.0 Panel-facing route manifest、代表性 JSON golden fixture 和 Go contract 形状测试已建立，已覆盖 Vision route removal、plugin null、非空 inbound users 字段和代表性错误 envelope；手动 contract drift 检查已接入，自动 release 监控仍未完成。
...
- [x] 官方 dev/2.8.0 已移除 `/vision/block-ip`、`/vision/unblock-ip` public contract；Go 侧同步不注册这些 route 并返回 404。
...
- [~] contract golden 回归矩阵已有官方 dev/2.8.0 route/request/response fixture、非空 inbound users 字段、plugin null、Vision removal 和代表性错误 envelope；已补官方 contract hash baseline，仍需继续扩展更多 fixture 变体。
