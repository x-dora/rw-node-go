...
- [x] 注册官方 [`3.0.0`](https://github.com/remnawave/node/tree/3.0.0) 计划内 Panel-facing routes。
- [x] 建立 `internal/contracts` 请求和响应类型。
- [x] 统一 response envelope。
- [~] 官方 3.0.0 Panel-facing route manifest、代表性 JSON golden fixture 和 Go contract 形状测试已建立，已覆盖 Vision route removal、plugin null、非空 inbound users 字段和代表性错误 envelope；手动 contract drift 检查已接入，自动 release 监控仍未完成。
...
- [x] 项目发布版本和 Panel 兼容版本已拆分：项目版本由根目录 `VERSION` 管理，Panel-facing `nodeVersion` 默认上报 `3.0.0`。
...
- [~] contract golden 回归矩阵已有官方 3.0.0 route/request/response fixture、非空 inbound users 字段、plugin null、Vision removal 和代表性错误 envelope；已补官方 contract hash baseline，仍需继续扩展更多 fixture 变体。
...
- [ ] 内嵌 `xray-core` 从 `v1.260327.0` 跟进到官方 3.0.0 pin 的 `v26.7.28`（上游该 tag 的 module path 仍是非 `/v26`，只能用 pseudo-version 固定 commit）；需要单独验证多架构构建和真机 stats/OnlineMap 全链路。
