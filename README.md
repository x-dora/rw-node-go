...
当前项目已完成基础协议层和部分运行时能力：公开路由、contract 类型、Gin HTTP 层、response envelope、CI、Docker 构建、mTLS/JWT/zstd、Xray 外部进程生命周期、内部 gRPC 健康检查、HandlerService 用户动态管理、基础 StatsService 流量统计、在线状态/IP 查询、宿主机 system stats、内部 Unix socket webhook 和 torrent blocker 报告链路已经接入。真实 Panel + Xray 验收、连接踢出、nftables 真实封禁等能力仍在后续阶段实现。
...
- [~] `/node/xray/start`、`/node/xray/stop`、`/node/xray/healthcheck` 已按官方缓存在线状态语义接入；Routing 业务方法仍未实现。
- [x] system stats 已按官方 2.7.0 响应形状返回宿主机 CPU、memory、uptime、load average、network interface 列表、默认网卡速率和 Xray sys stats。
- [~] Xray HandlerService 用户动态管理：add/remove/bulk、inbound users、inbound users count 已接入；真实 Panel + Xray 验收仍未完成。
- [~] Xray StatsService 统计：users、inbound、outbound、combined、online status、online IP list、reset 语义和官方错误 envelope 已接入；真实 Panel + Xray 验收仍未完成。
- [ ] drop users connections、drop IPs、Vision block/unblock 的真实实现。
- [~] torrent blocker plugin sync、官方 Unix socket webhook URL 注入、internal webhook 报告收集和 collect 返回已接入；真实 nftables 封禁仍未实现，报告中的 `blocked` 会明确返回 `false`。
- [ ] nftables 插件真实实现。
...
| `INTERNAL_SOCKET_PATH` | `/tmp/remnawave-node.sock` | 内部 webhook Unix socket 路径，用于 Xray 回调。 |
| `INTERNAL_REST_TOKEN` | 空 | 内部 webhook token；设置后 Xray webhook URL 会追加 `?token=...`。 |
...
启用 torrent blocker plugin 后，Xray 配置会追加 bittorrent blackhole outbound 和 webhook routing rule，webhook URL 采用官方 `/<INTERNAL_SOCKET_PATH>:/internal/webhook?token=<INTERNAL_REST_TOKEN>` 形态。内部 webhook 通过独立 Unix socket 接收官方 Xray webhook 形状的报告，供 `/node/plugin/torrent-blocker/collect` 拉取；主公网 API 不再裸豁免 `/internal/webhook`。
