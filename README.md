...
当前项目处于框架完成并进入 M3 收口的阶段：公开路由、contract 类型、Gin HTTP 层、response envelope、CI、Docker 构建、mTLS/JWT/zstd、Xray 外部进程生命周期、内部 gRPC 健康检查、HandlerService 用户动态管理、基础 StatsService 流量统计和宿主机 system stats 已经落地；在线 IP、连接踢出、torrent blocker、nftables 等真实运行时能力仍在后续阶段实现。
