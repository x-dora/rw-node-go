...
当前项目处于框架完成并进入部分 M1 实现的阶段：公开路由、contract 类型、Gin HTTP 层、response envelope、CI、Docker 构建、mTLS/JWT/zstd、Xray 外部进程生命周期和内部 gRPC 健康检查基础能力已经落地；HandlerService 用户管理、StatsService 流量统计、在线 IP、连接踢出、torrent blocker、nftables 等真实运行时能力仍在后续阶段实现。
...
- [~] `/node/xray/start`、`/node/xray/stop`、`/node/xray/healthcheck` 已按官方缓存语义接入；Handler/Stats/Routing 业务方法仍未实现。
