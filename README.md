...
当前项目处于框架完成并进入 M5 的阶段：公开路由、contract 类型、Gin HTTP 层、response envelope、CI、Docker 构建、mTLS/JWT/zstd、Xray 外部进程生命周期、内部 gRPC 健康检查、HandlerService 用户动态管理、基础 StatsService 流量统计、在线状态/IP 查询、宿主机 system stats 和 torrent blocker 报告链路已经落地；连接踢出、nftables 真实封禁等运行时能力仍在后续阶段实现。
...
- [~] Xray StatsService 统计：users、inbound、outbound、combined、online status、online IP list 和 reset 语义已接入；真实 Panel + Xray 验收仍未完成。
- [ ] drop users connections、drop IPs、Vision block/unblock 的真实实现。
- [~] torrent blocker plugin sync、Xray bittorrent webhook 配置注入、internal webhook 报告收集和 collect 返回已接入；真实 nftables 封禁仍未实现，报告中的 `blocked` 会明确返回 `false`。
- [ ] nftables 插件真实实现。
...
启用 torrent blocker plugin 后，Xray 配置会追加 bittorrent blackhole outbound 和 webhook routing rule，`/internal/webhook` 会收集官方 Xray webhook 形状的报告供 `/node/plugin/torrent-blocker/collect` 拉取。当前内部 webhook 仍复用主 Gin server，并对 JWT 做内部路径豁免；后续需要补独立 Unix socket 或本机 token 保护。
